...
function formatTime(minutes, seconds) {
  const m = minutes.toString().length > 1 ? minutes : '0'.concat(minutes);
  const s = seconds.toString().length > 1 ? seconds : '0'.concat(seconds);
...
function encodeToRot13(str) {
  ...
  str
    .split('')
    .forEach((el) =>
      table.length - table.indexOf(el) > 13
        ? arr.push(table[table.indexOf(el) + 13])
        : arr.push(table[13 - (table.length - table.indexOf(el))])
    );
  return arr.join('');
}
